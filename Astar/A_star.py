...
def draw_arrow(point,color='Grey',fill='black'):
    ...
    row_fat= int((rs[point.father_node.row]+rs[point.father_node.row+1])*0.5)
    col_fat= int((rs[point.father_node.col]+rs[point.father_node.col+1])*0.5)

    #the center coordinate of barth node, is also the beginning of the pole of arrow
    row_bir=int((rs[point.row]+rs[point.row+1])*0.5)
    col_bir=int((cs[point.col]+cs[point.col+1])*0.5)

    #the end of arrow
    row_end=int(0.8*row_bir+0.2*row_fat)
    col_end=int(0.8*col_bir+0.2*col_fat)
...
def draw_unit(point,color,flag=0):
    ...
    #show info
    point.f_set()
    t_f = tk.Label(window, text='f=%d' %point.f, bg='black', font=('Arial', np.max([1,int(np.floor(100/n))])), foreground='green', width=3, height=1 )
    t_g = tk.Label(window, text='g=%d' %point.g, bg='black', font=('Arial', np.max([1,int(np.floor(100/n))])), foreground='green', width=3, height=1 )
    t_h = tk.Label(window, text='h=%d' %point.h, bg='black', font=('Arial', np.max([1,int(np.floor(100/n))])), foreground='green', width=3, height=1 )
...
def l_callback(event):
    ...
    global start
    r = int(np.floor(event.y * len(map) / height))
    c = int(np.floor(event.x * len(map)/ width))
...
def hit1():
    ...
    if do == 1:
        ...
        n=int(text.get())
        tk.messagebox.showinfo(title='tips', message='鼠标左键选择开始位置;'+'\n'+
                                                     '鼠标中键选择障碍;'
                               +'\n' +'鼠标右键选择结束位置。 ')
        map_init()
        draw_init()
